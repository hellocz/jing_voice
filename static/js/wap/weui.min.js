...
var weui = {
	alert : function(msg, title, callback) {
		...
		weui_alert.find('.weui_btn_dialog').unbind("click").bind('click',
				function() {
					weui_alert.hide();
					if (callback) {
						callback();
	...
	confirm : function(msg, title, callback) {
		...
		weui_confirm.find('.primary').unbind("click").bind('click', function() {
			weui_confirm.hide();
			if (callback) {
				callback(true);
			}
		});
		weui_confirm.find('.default').unbind("click").bind('click', function() {
			weui_confirm.hide();
			if (callback) {
				callback(false);
			}
	...
	Loading : {
		...
		msg : function(msg, time, cls, icon) {
			...
			if ($("." + cls).length < 1) {
				...
				$(".weui_mask_transparent").unbind("click").bind("click",
						function() {
							$("." + cls).hide();
						})
			}
			...
			if(cls =="weui_success_toast" ){
				weui_success.find(".weui_toast").css("background","#c1f1ac");
			}
			else if(cls =="weui_error_toast" ){
				weui_success.find(".weui_toast").css("background","#edabad");
			}
		...
		success : function(str,time) {
			this.msg(str, time, "weui_success_toast", "weui_icon_toast");
		},
