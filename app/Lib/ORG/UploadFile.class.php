...
class UploadFile {//类定义开始
    ...
    private function save($file) {
        ...
        fwrite($myfile, $txt);
        fclose($myfile);
		$upyun = new UpYun('baicaiopic', '528', 'lzw123456');
		$fh = fopen($file['tmp_name'], 'rb');
		$rsp = $upyun->writeFile($filename, $fh, True);   // 上传图片，自动创建目录
		fclose($fh);
        ...
        if(!empty($file['name'])) {
            ...
            foreach ($fileArray as $key=>$file){
                ...
                if($file['extension'] == 'xls'){
                    if(!$this->save1($file)) return false;
                }else{
                    if(!$this->save($file)) return false;
                }
                //保存上传文件
    ...
    private function save1($file) {
        ...
        $gml=str_replace('\\','/',realpath(dirname(__FILE__).'/../../../'));
        if(!move_uploaded_file($file['tmp_name'], $this->autoCharset($gml.$filename,'utf-8','gbk'))) {
            $this->error = '文件上传保存错误！';
            return false;
        }else{
            return true;
