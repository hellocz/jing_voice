...
class messageAction extends userbaseAction {
    ...
    public function index() {
        ...
        if ($count) {
            ...
            $res_list = $message_mod->field('MAX(id) as id,COUNT(id) as num')->where($map)->group('ftid')->order('id DESC')->limit($start.','.$pagesize)->select();
            $talks = array();
		...
		if($more == 'more'){
			$more_arr="";
			foreach($talk_list as $key=>$r){
				$df=$r['from_id']==$uid ? L('i_to_h') : L('h_to_i');
				$more_arr.="<li><a href='".U('wap/space/index', array('uid'=>$r['ta_id']))."' title='".$r['ta_name']."'><div class='w_zk_img'>";
				$more_arr.="<img src='".avatar($r['ta_id'],48)."' title='$r[ta_name]' alt='$r[ta_name]' /></div>";
				$more_arr.="<address><span>".fdate($r['add_time'])."</span>".$r['ta_name']."</address></a><h2>".$df;
				$more_arr.=":<a href='".U('wap/message/talk', array('ftid'=>$r['ftid']))."'>".$r['info']."</a></h2>";
				$more_arr.="<div class='w_jg'><div class='w_gbjy' onclick='window.location=\"wap-message-talk-ftid-".$r['ftid']."\"' style='margin-left:10px; width:110px;'>";
				$more_arr.=sprintf(L('see_all_talk'),$r['num'])."</div></div></li>";
			}
			echo $more_arr;
			exit;
		}
    ...
    public function delall() {
        ...
        $this->redirect('wap/message/index');
    }
    ...
    public function system() {
        ...
        //$pager = $this->_pager($count, $pagesize);
        $system_list = $message_mod->where($map)->order('id DESC')->limit($start.','.$pagesize)->select();
        D('user_msgtip')->clear_tip($uid, 4);
		if($more == 'more'){
			$more_arr="";
			foreach($system_list as $key=>$r){
				$more_arr.="<li><div>".fdate($r['add_time'])."</div><p>".$r['info']."</p></li>";
			}
			echo $more_arr;
			exit;
		}
