    ...
    */
    public function index() {
        if (IS_POST) {
            ...
            $tpl_data['reset_url'] = U('wap/findpwd/reset', $url_args, '', '', true);
            //解析邮件模板
    ...
    public function reset() {
        ...
        //判断是否已经过期
        $time = time();
        ($time - $t) > 3600 && $this->error(L('findpwd_link_expired'), U('wap/findpwd/index'));
        //验证用户
        $user = M('user')->field('id,reg_time,password')->where(array('username'=>$username))->find();
        !$user && $this->error(L('username').L('not_exist'), U('wap/index/index'));
        if ($activation != md5($user['reg_time'] . substr($user['password'], 10) . $t)) {
            $this->error(L('findpwd_link_error'), U('wap/index/index'));
        }
        if (IS_POST) {
            $captcha = $this->_post('captcha', 'trim');
            session('captcha') != md5($captcha) && $this->error(L('captcha_failed'));
            ...
            $this->success(L('reset_password_success'), U('wap/user/login'));
        } else {
            $this->assign('title_h2','重置密码');
			$this->_config_seo();
            $this->display();
        }
    }
