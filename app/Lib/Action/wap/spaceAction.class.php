...
class spaceAction extends frontendAction {
    ...
    public function index() {
		...
		!$t&&$t="gn";
		if($t=="gn"){$ismy="0";}else{$ismy="1";}
        //用户的分享
		$mod=M("item");
		$pagesize=10;
		$start=($p-1)*$pagesize;
		$uid=$this->_user['id'];
		$count = $mod->where("i.uid='$uid'")->join(" as i left join try_item_orig as o ON o.id=i.orig_id")->count();
		//$pager = $this->_pager($count,$pagesize);
		$list = $mod->query("select i.* from __TABLE__ as i left join try_item_orig as o ON o.id=i.orig_id where i.uid='$uid'  order by i.isbest,i.ordid,i.id desc limit ".$start.",".$pagesize);
