...
class daoAction extends frontendAction{
	public function index(){
		...
		$list = M("zzposts")->field("id,post_author,post_date,post_title,post_content,post_data")->where("post_type='post' and post_status='publish' and id=48")->limit(1)->select();
		 foreach($list as $key=>$val){
			 $data=array();
			 $data['id']=$val['id'];
			 $data['add_time']=strtotime($val);
