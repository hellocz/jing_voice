...
<div class="w_gezx_ce"> 
  <div class="w_ger">
	...
	<div class="w_yhjj">
	 <h2>我的优惠劵</h2> 
     <div class="w_yhjj_t"><em><a href="<?php echo U('tick/index');?>" title="领取优惠劵">领取优惠劵</a></em><span class="w_yhjj_t_s">兑换的优惠劵</span></div>
     <div class="w_yhjj_t1"><em></em>筛选：<span <?php if($tab == ''): ?>class="w_yhjj_t1_s"<?php endif; ?> onclick="window.location='<?php echo U('user/tick');?>'">全部（<?php echo ($all); ?>）</span><span <?php if($tab == '1'): ?>class="w_yhjj_t1_s"<?php endif; ?> onclick="window.location='<?php echo U('user/tick',array('gq'=>1));?>'">已过期（<?php echo ($gq); ?>）</span></div>
      <ul class="w_yhj_nr">
	  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><li>
          <a href="<?php echo U('tick/show',array('id'=>$item['id']));?>" title="<?php echo ($item['name']); ?>"><img src="<?php echo orig_img($item['orig_id']);?>"/></a>
          <div><a href="<?php echo U('tick/show',array('id'=>$item['id']));?>" title="<?php echo ($item['name']); ?>"><?php echo ($item['name']); ?><br/>有效时间:<?php echo ($item["end_time"]); ?></a></div>
            <div class="ssnx">
                <p>券码/密码<?php echo ($item["tk_code"]); if(!empty($item['tk_psw'])): ?>/<?php echo ($item["tk_psw"]); endif; ?></p>
                <p>
				
				  <a href="<?php if($item['sssss'] == 1): ?>javascript:;<?php else: echo ($item["ljdz"]); endif; ?>" title="立即兑换" target="_blank"  class="w_dha" <?php if($item['sssss'] == 1): ?>style="background: #c9c9c9;"<?php endif; ?>>去使用</a>
