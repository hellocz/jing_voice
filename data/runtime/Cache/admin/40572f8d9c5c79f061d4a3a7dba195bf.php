	...
	<div class="col_tab">
		...
		<table width="100%" cellpadding="2" cellspacing="1" class="table_form">
			...
			<tr>
				...
				<th width="120">从站内url获取 :</th>
				<td>
				<input type="text"  id="J_fromUrl" class="input-text" size="100">
				<input type="button" value="<?php echo L('auto_from_url');?>" id="J_getfromUrl" name="fromUrl_btn" class="btn">
				</td>
			</tr>
			<tr>
                ...
                <td><select class="J_cate_select mr10" data-pid="0" data-uri="<?php echo U('item_cate/ajax_getchilds', array('type'=>0));?>" data-selected=""></select>

				<input type="hidden" name="cate_id" id="J_cate_id" value="" /><b style="color: orangered; font-weight: bold;">* 必填项</b></td>
			...
			<tr>
				...
				<td><input type="text" name="otitle" id="J_otitle" class="input-text" size="60"></td>
...
<!--
			<tr>
                ...
                <td><textarea name="intro" id="intro" cols="80" rows="2"></textarea></td>
...
-->
            <tr>
				...
				<td>
				<div class="w_tpk"><img id="btn_img" src="<?php if($item['img'] != '' && $item['img']!='http:' ): echo ($item['img']); else: ?>/images/jia_pic.png<?php endif; ?>"/>
		...
		<!--		<input type="file" name="img"  id="img" class="w_bl_in2" /
				<input type="text" size="40" name="img_usb"   placeholder="远程图片地址" class="input-text" value="">
> -->
		</div>
...
<span style="color:red;"><分隔符支持中文的顿号、或者英文的逗号,></span>
            ...
            <tr>
				...
				<td><input type="text" name="price" id="price" class="input-text" size="50"> </td>
			...
			<!--<tr>
				...
				<td><input type="text" name="express" id="express" class="input-text" size="10"> 元 <label><input type="checkbox" value="1" name="ispost" id="ispost" >可直邮</label></td>
			...
			<tr>
				...
				<td>
					...
					<label><input type="checkbox" value="1" name="isbest" id="isbest" <?php if($info['isbest'] == 1): ?>checked<?php endif; ?>>精品</label> &nbsp;&nbsp;

					<label><input type="checkbox" value="1" name="ishot" id="ishot" <?php if($info['ishot'] == 1): ?>checked<?php endif; ?>>热门</label> &nbsp;&nbsp;

					<label><input type="checkbox" value="1" name="istop" id="istop" <?php if($info['istop'] == 1): ?>checked<?php endif; ?>>置顶</label> &nbsp;&nbsp;

					<label><input type="checkbox" value="1" name="ispost" id="ispost" <?php if($info['ispost'] == 1): ?>checked<?php endif; ?>>可直邮</label> &nbsp;&nbsp;
			...
			<tr>
				...
				<td>
					...
					<script id="content" name="content" type="text/plain" style="width:80%;height:300px;"></script>

					<!--<textarea name="content" id="content" style="width:68%;height:400px;visibility:hidden;resize:none;"></textarea>--></td>
			...
			<input type="button" style="display: none" value="<?php echo L('rm_ds_publish');?>" id="J_rm_publish" name="tags_btn" class="btn"></th>
				...
				<input type="text"  style="display: none" name="add_time" id="add_time" size="25" class="input-text" value="">
			...
			<!--<tr>
				...
				<td>
					...
					<input type="button" id="save_msg"  class="btn btn_submit" onclick="ajx()" value="<?php echo L('submit');?>" value="保存草稿" class="btn">&nbsp;&nbsp;&nbsp;&nbsp;

					<input type="button" id="send_msg"  class="btn btn_submit" onclick="ajx()" value="<?php echo L('submit');?>" value="直接发布" class="btn">
		...
		<!--<div class="mt10"><input type="button" onclick="ajx()" value="<?php echo L('submit');?>" class="btn btn_submit"></div>-->

		<div class="mt10">

			<input type="button" id="save_msg" style="margin:0 0 10px 100px;"  class="btn btn_submit" onclick="ajx1()" value="保存草稿" class="btn">&nbsp;&nbsp;&nbsp;&nbsp;

			<input type="button" id="send_msg" style="margin:0 0 10px 10px;" class="btn btn_submit" onclick="ajx()" value="直接发布" class="btn">&nbsp;&nbsp;&nbsp;&nbsp;
			<input type="button"  class="btn btn_submit" id="preview" name="dosubmit" onclick="previewx()" style="margin:0 0 10px 10px;"   value="预览" class="btn">
...
$(function() {
	...
	$('#J_getfromUrl').live('click', function() {

		var url = $.trim($('#J_fromUrl').val());

		if(url == ''){

			$.pinphp.tip({content:lang.article_url_isempty, icon:'alert'});

			return false;

		}

		$.getJSON('<?php echo U("item/ajax_getfromurl");?>', {url:url}, function(result){

			if(result.data.status == 1){
				$(".J_cate_select").cate_select(2);
				$('#J_title').val(result.data.title);
				$('#J_otitle').val(result.data.otitle);
				//$('#intro').val(result.data.intro);
				$('#J_link').val(result.data.go_link);
				$('#J_tags').val(result.data.tag_cache);
				$('#price').val(result.data.price);
				$('#orig_id').val(result.data.orig_id);
				$('#img').val(result.data.img);
				$('#btn_img').attr('src',result.data.img);
	...
	$("#J_title").blur(function(){
		...
		$.getJSON('<?php echo U("item/add");?>', {title:title,id:id}, function(result){
...
$("#J_add_link").live('click',function(){
	...
	$("#Link_form").append('<span><br><br><input name="link_type[]" value="直达链接"  class="input-text"> ：<input type="text" name="link_url[]"  value="" class="input-text"/>&nbsp;&nbsp;<input type="button" value=" - " id="J_del_link"/></span>');
	...
	if($('#J_cate_id').val()== 0){
		  ...
		  alert("请上传商品图片！");
			...
			success: function(data) {
				...
				location.href ="?g=admin&m=item&a=index&menuid=52";
...
<script src="/js/function_item_acg.js"></script>
