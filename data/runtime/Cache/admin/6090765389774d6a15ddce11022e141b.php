...
<div class="pad_lr_10" >
        ...
        <tbody>
            ...
            <tr>
                ...
                <div class="explain_col">
                    ...
                    <option value="2" <?php if($search["status"] == '2'): ?>selected="selected"<?php endif; ?>>草稿</option>

                    <option value="3" <?php if($search["status"] == '3'): ?>selected="selected"<?php endif; ?>>退回</option>

                    <option value="4" <?php if($search["status"] == '4'): ?>selected="selected"<?php endif; ?>>删除</option>
                    ...
                    <select name="item_type">
                        ...
                        <option value="isbest" <?php if($search["item_type"] == 'isbest'): ?>selected="selected"<?php endif; ?>>精品</option>

                        <option value="ishot" <?php if($search["item_type"] == 'ishot'): ?>selected="selected"<?php endif; ?>>热门</option>

                        <option value="istop" <?php if($search["item_type"] == 'istop'): ?>selected="selected"<?php endif; ?>>置顶</option>

                        <option value="isbao" <?php if($search["item_type"] == 'isbao'): ?>selected="selected"<?php endif; ?>>网友爆料</option>

                        <option value="isnice" <?php if($search["item_type"] == 'isnice'): ?>selected="selected"<?php endif; ?>>编辑推荐</option>
	...
	<th align="left"><span data-tdtype="order_by" data-field="title">来源</span></th>
    	...
    	<tbody>
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td align="right">
                    ...
                    <?php if(!empty($val['img'])): ?><div class="img_border"><img src="<?php echo attach($val['img'],'item');?>" width="32" width="32" class="J_preview" data-bimg="<?php echo attach($val['img'], 'item');?>"></div><?php endif; ?>
				...
				<td>
					...
					<?php if($val["source"] == 0): ?><font color="#FF0000">管理员</font>
				...
				<?php if($val["status"] == 1): if($val['ds_time'] > time() ): ?><font style="color:#FF00FF">定时</font>

					<?php else: ?>

						<font color="green">通过</font><?php endif; ?>

				<?php elseif($val["status"] == 0): ?><font color="red">未审</font>

				<?php elseif($val["status"] == 3): ?><font color="red">退回</font><?php elseif($val["status"] == 2): ?><font color="orange">草稿</font><?php elseif($val["status"] == 4): ?><font color="blue">删除</font><?php endif; ?></td>

                <td align="center">
                <?php if($val['add_time'] == 0 ): ?>未定
                <?php else: ?>
                <?php echo (date('Y-m-d H:i:s',$val["add_time"])); endif; ?>
                </td>

                <td align="center"><!--<?php if($val['status'] != 1 and $val['status'] != 2): ?><a href="javascript:;" class="J_showdialog " data-uri="/?g=admin&m=item&a=set_score&id=<?php echo ($val["id"]); ?>" data-title="设置积分信息" data-id="edit" data-acttype="ajax" data-width="300" data-height="140" title="审核">审核</a> |<?php endif; ?>-->

                    <?php if(sbbtime($val['status'])): endif; ?>

                    <a href="<?php echo u('item/edit', array('id'=>$val['id'], 'menuid'=>$menuid));?>">审核</a> | <a href="javascript:void(0);" class="J_confirmurl" data-uri="<?php echo u('item/delete', array('id'=>$val['id']));?>" data-acttype="ajax" data-msg="<?php echo sprintf(L('confirm_delete_one'),$val['title']);?>"><?php echo L('delete');?></a></td>
